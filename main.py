...
# Write header to csv
    ...
    while (pages > 0):
        ...
        pages = pages + 1
        newpage = '?page=' + str(pages);
        with open("hosts.txt") as json_file:
            data = json.load(json_file)
            if (pages == 2):
...
def add2group(groupids, hostfile):
    global token  #
    ...
    print(curlarg)
    curlarg = 'curl -H "Authorization: Bearer ' + token + '" --insecure -X GET "https://ansible-tower.ocp1.sr1.eu1.sp.ibm.local/api/v2/groups/' + groupids + '/" 2> null'
...
#        filename = filename.replace('/','\\')
    ...
    if event == 'Add hosts':
       ...
       print("add hosts")
#       continue
       add2group(group_dic[combo], filename)
